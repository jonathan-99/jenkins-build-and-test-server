#! /bin/bash
#
# Script to install a docker image of Jenkins on either an Ubuntu or Raspberry pi.
# Error with installation on to Raspberry Pi states, no docker image available for "arm v7" 01/01/24.
#
...
# wouterds/rpi-jenkins, jenkins-arm64
...
echo "$variable_2"
echo cat "$variable_2" | grep -e "oscar"
